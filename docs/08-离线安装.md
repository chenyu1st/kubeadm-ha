...
- 下载所需离线包
  ...
  # 二进制基础软件包
  curl -LO https://files.saas.hand-china.com/kubeadm-ha/kubernetes-1.31.12-base-amd64.tgz
  # 集群运行所需的镜像
  curl -LO https://files.saas.hand-china.com/kubeadm-ha/kubernetes-1.31.12-images-amd64.tgz
  # CentOS 7 系统对应软件包
  curl -LO https://files.saas.hand-china.com/kubeadm-ha/centos7/kubernetes-1.31.12-centos7-amd64.tgz
  # 克隆脚本
  git clone -b release-1.31 https://github.com/TimeBye/kubeadm-ha.git
  ```
...
- 准备 kubeadm-ha(ansible) 环境
  - 解压离线包
    ```shell
    tar -xzvf kubernetes-1.31.12-base-amd64.tgz
    tar -xzvf kubernetes-1.31.12-images-amd64.tgz
    tar -xzvf kubernetes-1.31.12-centos7-amd64.tgz
    ```
  - CentOS 类系统添加软件源
...
- 使用 containerd 运行 kubeadm-ha 镜像（与 docker 安装二选一）
  ...
  - 配置 containerd
    ...
    [plugins]
      [plugins."io.containerd.grpc.v1.cri"]
        sandbox_image = "registry.custom.local:12480/kubeadm-ha/pause:3.10"
  ...
  - 配置本地 http 仓库
    ```
    mkdir -p /etc/containerd/certs.d/registry.custom.local:12480

    cat <<EOF | tee /etc/containerd/certs.d/registry.custom.local:12480/hosts.toml
    server = "https://registry.custom.local:12480"
    [host."http://registry.custom.local:12480"]
      skip_verify = true
